...
const std::shared_ptr<ManagementInterface> ServiceController::get_management_interface() const {
...
void ServiceController::connect(const PolycubedCore *core) {
  switch (type_) {
  ...
  case ServiceControllerType::LIBRARY: {
    management_interface_ =
        std::make_shared<ManagementLib>(servicecontroller_, base_url_, name_, core);
