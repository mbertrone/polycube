...
class AbstractFactory {
 public:
  ...
  std::unique_ptr<CaseResource> BodyCase(const std::string &name,
  ...
  std::unique_ptr<ParentResource> BodyGeneric(
      const std::string &name, const std::string &description, const std::string &cli_example,
      const ParentResource *const parent, const std::vector<JsonNodeField> &node_fields,
      bool configuration, bool init_only_config, bool container_presence) const;

 protected:
  const PolycubedCore *const core_;
  explicit AbstractFactory(const PolycubedCore *core);
};
}  // namespace polycube::polycubed::Rest::Resources::Body
