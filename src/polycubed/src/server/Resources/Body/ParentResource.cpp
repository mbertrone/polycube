...
ParentResource::ParentResource(
    ...
    const PolycubedCore *core, const std::vector<JsonNodeField> &node_fields,
...
std::vector<Response> ParentResource::BodyValidate(const std::string &cube_name,
    ...
    if (std::dynamic_pointer_cast<ChoiceResource>(child) != nullptr) {
      ...
      if (body_copy.count(child_name) == 0) {
        ...
        if (!child->IsConfiguration() ||
          ...
          auto child_errors =
              child->BodyValidate(cube_name, keys, body_copy[child_name],
                                  initialization);
  ...
  for (const auto &choice : choices) {
    ...
    auto choice_errors = choice->BodyValidate(cube_name, keys, body_copy,
                                              initialization);
